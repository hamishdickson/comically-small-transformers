...
train_data, val_data = create_dataloaders(text, tokenizer, model_config.block_size, batch_size=training_config.batch_size)
...
scheduler = transformers.get_cosine_schedule_with_warmup(
    ...
    num_training_steps=training_config.total_steps if training_config.total_steps > 0 else len(train_data) * training_config.epochs
)
...
kickoff = torch.tensor(tokenizer.encode("write me the python code for Einstein's equation of general relativity"), dtype=torch.long).to(device).unsqueeze(0)
